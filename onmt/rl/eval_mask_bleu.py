...
# vocab = load_txt("../data_reddit_small/vocab.txt")
# HIGH_VOCAB = set(["<unk>"] + vocab[:150])
HIGH_VOCAB = set([str(i) for i in range(154)])
MODE = "low"
...
def ngrams(sequence, n, pad_left=False, pad_right=False,
           left_pad_symbol=None, right_pad_symbol=None, infer=False):
    ...
    for item in sequence:
        history.append(item)
        #
        if MODE and not infer:
            flag = False
            for word in history:
                if MODE == "high":
                    if word not in HIGH_VOCAB:
                        flag = True
                        break
                else:
                    if word in HIGH_VOCAB:
                        flag = True
                        break
            if flag:
                del history[0]
...
def modified_precision(references, hypothesis, n):
    ...
    counts = Counter(ngrams(hypothesis, n, infer=True)) if len(hypothesis) >= n else Counter()
...
def eval_bleu(ref_resp, hyps_resp):
    ...
    bleu = corpus_bleu(ref_resp, hyps_resp, smoothing_function=SmoothingFunction().method1)
    return bleu
...
if __name__ == '__main__':
    ...
    golden_path = "../data_reddit_small/tgt-test.txt"
    infer_path = "./infer.txt"
    # global MODE
    # MODE = "low"
