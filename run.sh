...
if [ "$2" = "preprocess" ]; then
  ...
  CUDA_VISIBLE_DEVICES="$1" python3 rl_train.py -gpu 0 -model "$3" -sample_method "$4" -rl_samples 4 -epochs 2 -report_every 10 -valid_steps 50 -save_checkpoint_steps 100 -warmup_steps 250 -learning_rate 0.0001 -learning_rate_decay 0.75 -start_decay_steps 250 -decay_steps 250 -save_model rl_checkpoint/mlp_"$4"_"$5" -data "$DATA_DIR"/"$DATASET" -src "$DATA_DIR"/src-train.txt -tgt "$DATA_DIR"/tgt-train.txt -valid_src "$DATA_DIR"/src-valid.txt -valid_tgt "$DATA_DIR"/tgt-valid.txt -tag_src "$DATA_DIR"/"$DATASET"-src-train.txt -tag_tgt "$DATA_DIR"/"$DATASET"-tgt-train.txt -valid_tag_src "$DATA_DIR"/"$DATASET"-src-valid.txt -valid_tag_tgt "$DATA_DIR"/"$DATASET"-tgt-valid.txt -optim adam -output result/rl_"$DATA_DIR"_"$DATASET"_"$4"_"$5".txt -beam 1 -batch_size 1024 -max_length 30
