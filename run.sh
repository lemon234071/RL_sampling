...
if [ "$2" = "preprocess" ]; then
  ...
  python3 translate.py -gpu "$1" -model "$3" -sample_method "$4" -tag_mask "$DATA_DIR"/"$DATASET"_mask.json -output result/"$DATA_DIR"_"$DATASET"_"$4".txt -beam 1 -batch_size 128 -src "$DATA_DIR"/src-test.txt -max_length 30 -tag_src "$DATA_DIR"/"$DATASET"-src-test.txt
