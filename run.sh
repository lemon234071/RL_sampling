...
if [ "$2" = "preprocess" ]; then
  ...
  if [ "$3" = "seq2seq" ]; then
    ...
    CUDA_VISIBLE_DEVICES="$1" python3 train.py -mask_attn -tag_gen "$4" -data "$DATA_DIR"/"$DATASET" -save_model checkpoint/"$DATA_DIR"_freq_"$4"_"$5" -log_file ./log_dir/"$DATA_DIR"_freq_"$4"_"$5".log -world_size 1 -gpu_ranks 0 -global_attention mlp -word_vec_size 256 -rnn_size 512 -optim adam -param_init_glorot -rnn_type GRU -start_decay_steps 10000 -learning_rate 0.001 -train_steps 50000 -batch_size 128 -max_generator_batches 128 -report_every 500 -pos_vec_size 128 -valid_steps 1000 -save_checkpoint_steps 5000 -statistic
  else
