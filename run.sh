...
if [ "$2" = "preprocess" ]; then
  ...
  if [ "$3" = "seq2seq" ]; then
    ...
    CUDA_VISIBLE_DEVICES="$1" python3 train.py -mask_attn -tag_gen "$4" -generators 1:154,0:49850 -itoj "$DATA_DIR"/freq_itoj.json -data "$DATA_DIR"/"$DATASET" -save_model checkpoint/"$DATA_DIR"_freq_"$4"_"$5" -log_file ./log_dir/"$DATA_DIR"_freq_"$4"_"$5".log -world_size 1 -gpu_ranks 0 -global_attention mlp -word_vec_size 256 -rnn_size 512 -optim adam -param_init_glorot -rnn_type GRU -start_decay_steps 10000 -learning_rate 0.001 -train_steps 50000 -batch_size 128 -max_generator_batches 128 -report_every 500 -pos_vec_size 128 -valid_steps 1000 -save_checkpoint_steps 5000 -statistic
  elif [ "$3" = "tri" ]; then
    CUDA_VISIBLE_DEVICES="$1" python3 train.py -generators stop:152,ord:13862,vn:35990 -mask_attn -tag_gen "$4" -itoj "$DATA_DIR"/tri_itoj.json -data "$DATA_DIR"/"$DATASET" -save_model checkpoint/"$DATA_DIR"_"$DATASET"_"$4"_"$5" -log_file ./log_dir/"$DATA_DIR"_"$DATASET"_"$4"_"$5".log -world_size 1 -gpu_ranks 0 -global_attention mlp -word_vec_size 256 -rnn_size 512 -optim adam -param_init_glorot -rnn_type GRU -start_decay_steps 10000 -learning_rate 0.001 -train_steps 50000 -batch_size 128 -max_generator_batches 128 -report_every 500 -pos_vec_size 128 -valid_steps 1000 -save_checkpoint_steps 5000 -statistic
  else
    echo "Input arg 3 Is Error: $3."
  fi
elif [ "$2" = "infer" ]; then
  python3 translate.py -gpu "$1" -model "$3" -sample_method "$4" -tag_mask -tag_mask "$DATA_DIR"/"$DATASET"_mask.json -output result/"$DATA_DIR"_"$DATASET"_"$4".txt -beam 1 -batch_size 128 -src "$DATA_DIR"/src-test.txt -max_length 30 -tag_src "$DATA_DIR"/"$DATASET"-src-test.txt
elif [ "$2" = "rl_train" ]; then
  CUDA_VISIBLE_DEVICES="$1" python3 rl_train.py -gpu 0 -model "$3" -sample_method "$4" -generators stop:152,ord:13862,vn:35990 -tag_mask "$DATA_DIR"/"$DATASET"_mask.json -rl_samples 4 -epochs 2 -batch_size 1024 -report_every 5 -valid_steps 20 -save_checkpoint_steps 40 -random_steps 40 -learning_rate 0.01 -learning_rate_decay 0.75 -start_decay_steps 30 -decay_steps 30 -save_model rl_checkpoint/mlp_"$4"_"$5" -data "$DATA_DIR"/"$DATASET" -src "$DATA_DIR"/src-train.txt -tgt "$DATA_DIR"/tgt-train.txt -valid_src "$DATA_DIR"/src-valid.txt -valid_tgt "$DATA_DIR"/tgt-valid.txt -tag_src "$DATA_DIR"/"$DATASET"-src-train.txt -tag_tgt "$DATA_DIR"/"$DATASET"-tgt-train.txt -valid_tag_src "$DATA_DIR"/"$DATASET"-src-valid.txt -valid_tag_tgt "$DATA_DIR"/"$DATASET"-tgt-valid.txt -optim adam -output result/rl_"$DATA_DIR"_"$DATASET"_"$4"_"$5".txt -beam 1 -max_length 30
